...
type NewSneaker = { name: string; brand: string; subcategory?: string; priceRange: [number, number] };

const newSneakers: NewSneaker[] = [
...
async function main() {
  const sneakers = await db.category.findFirstOrThrow({ where: { slug: "sneakers" } });
    ...
    for (let i = 0; i < numListings; i++) {
      ...
      const listing = await db.listing.create({
        data: {
          ...
          status,
          size: pick(["UK6", "UK7", "UK8", "UK9", "UK10", "UK11"]),
          quantity: 1,
        },
      });

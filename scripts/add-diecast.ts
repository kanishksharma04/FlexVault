...
type NewDiecast = { name: string; brand: string; priceRange: [number, number] };

const newDiecast: NewDiecast[] = [
  { name: "Nissan Skyline GT-R R34 Z-Tune", brand: "Tarmac Works", priceRange: [4000, 8000] },
  { name: "Toyota GR Supra LB-Works", brand: "Mini GT", priceRange: [2800, 5500] },
];

async function main() {
  const diecast = await db.category.findFirstOrThrow({ where: { slug: "diecast" } });

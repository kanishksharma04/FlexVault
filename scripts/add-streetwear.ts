...
type NewStreetwear = { name: string; brand: string; subcategory?: string; priceRange: [number, number] };

const newStreetwear: NewStreetwear[] = [
  { name: "Alcatraz Hoodie Triple Black", brand: "Corteiz", subcategory: "Hoodies", priceRange: [12000, 20000] },
  { name: "Cotton Wreath Sweatshirt Black", brand: "Denim Tears", priceRange: [15000, 25000] },
];

async function main() {
  const streetwear = await db.category.findFirstOrThrow({ where: { slug: "streetwear" } });
  ...
  for (const item of newStreetwear) {
